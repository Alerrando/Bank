    ...
    <>
      ...
      <main
        className={`w-screen h-[calc(100vh-_64px)] grid grid-rows-login mt-[64px] 
                ${access ? "md:grid-cols-inputs-register" : "md:grid-cols-[60%_40%]"} md:grid-rows-none overflow-y-auto overflow-x-hidden animate-${access}`}
      >
        <aside className={`h-full md:h-5/6 w-full flex items-center justify-center my-auto relative ${access && "md:order-1"} overflow-hidden`}>
          <Image src={access ? "/aside-login.svg" : "/aside-register.svg"} alt="" className="w-full md:w-10/12 h-full mx-auto" fill={true} />
        </aside>
